...
public class VKManager {
    ...
    public void sendMessage(String msg, int peerId){
        ...
        try {
            System.out.println("бот ответил "+ MyJSONParser.getUserRealNameById(peerId+"")+": " + msg);
    ...
    public void sendPhoto(Message message, int peerId){
        vkCore.getVk().messages().send(vkCore.getActor()).peerId(peerId).attachment(message.getPhoto100());
    }
    public void sendMessageToChat(String msg, int chatId){
        if (msg == null){
            System.out.println("null");
            return;
        }
        try {
            vkCore.getVk().messages().send(vkCore.getActor()).chatId(chatId).message(msg).execute();
        } catch (ApiException | ClientException e) {
            e.printStackTrace();
        }
    }
